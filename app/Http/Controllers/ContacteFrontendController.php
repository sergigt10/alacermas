...
class ContacteFrontendController extends Controller
{
    /**
    ...
    public function sendContacte(Request $request)
    {
        ...
        if($captcha != ''){
            ...
            if($array['success']){
                ...
                $AdminMessage .= "Empresa: ".utf8_decode($data['empresa'])."\n";
                $AdminMessage .= "Localidad: ".utf8_decode($data['localidad'])."\n";
                $AdminMessage .= "Pais: ".utf8_decode($data['pais'])."\n";
                $AdminMessage .= "Activitat: ".utf8_decode($data['actividad'])."\n";
                $AdminMessage .= "Departament: ".utf8_decode($data['departamento'])."\n";
                ...
                if(isset($_FILES) && (bool) $_FILES) {
                    ...
                    $from = $_POST['mail']; 
                    $subject ="Formulari de contacte WEB"; 
                    $message = "Formulari de contacte WEB";
                    $headers = "From: $from";
                    ...
                    mail("phxhollow13@hotmail.com", "Formulari de contacte WEB", $AdminMessage, "From: ".$data['mail']);
                    return back()->with(['message_mail' => trans('Mensaje enviado correctamente! En breve nos pondremos en contacto con usted. Gracias.')]);
                }
            }else{
                // bot pasar
